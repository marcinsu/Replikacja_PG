...
create or replace function replikuj_formy_platnosci() returns trigger as $$ -- $$ lub $body$ oznacza ciało funkcji/metody, tzn. rozpoczęcie procedury tak jak widać
	declare --deklaracja
		nil text; -- zmienna typu tekst
		-- nil to przykładowa nazwa, ponieważ funkcie dblink_connect, dblink_exec wracają jakieś informacje typu udało się OK,
		-- lub niepowodzenie trzeba użyć jekieś zmiennej która będzie odbireać te dane (trigger nie może otrzymywać takich danych) wszystkie funkcje muszą być 
	...
	begin -- rozpoczęcie procedury
		...
		-- old - przechowuje dane typu record po wykonaniu polecenia delete lub update, w przypadku update będzie zawierać pola/kolumny wiersza które zostały zmodyfikowane (przechowuje wcześniejsze dane kolumn wierasza)
		-- jeśli mamy kolumne o nazwie "platnosc" to wartość przed modykikacją będzie dostępna przez zapis - old.platnosc, na nowa wartość - new.platnosc
		if (TG_OP = 'UPDATE') then -- warunek dla przypadku kiedy trigger został wywołany przez update
			if (new.typ_platnosci is not null) then -- sprawdzamy czy new.jakas_nazwa została zmodyfikowana, jeśli nie to jest o wartości null
				-- wywołujemy polecenie update na połączeniu r_formy_platnosci modyfikując kolumne na tabeli r_formy_platnosci w bazie hubert
		...
		exception -- obsługa wyjątków, w przypdku jakiegoś nieprzewidzianego błędu
			when others then 
				select dblink_disconnect('r_formy_platnosci') into nil; -- jeśli wcześniej nawiązano połączenie należy je zakończyć inaczej utworzenie 
																		-- ponowne pod taką samą nazwą nie będzie możliwe, a co za tym idzie trigger replikacji przeztanie działać i za każdym razem będzie zwracać błąd
...
create trigger t_replikuj_formy_platnosci -- tworzenie trziggera, wywływacza
...
execute procedure replikuj_formy_platnosci(); -- wykonuje procedure replikuj_formy_platnosci(), czyli to co wyżej gdzie jest "create or replace function replikuj_formy_platnosci() returns trigger as $$" a kończy się na "$$ language plpgsql;"
